...
const Carousel = memo(
  ({
    ...
    const cylinderWidth = isScreenSizeSm ? 600 : 800
    ...
    }, [controls, faceCount, isCarouselActive, rotation]);
    ...
    return (
      ...
      >
        <motion.div
          ...
          onDrag={(_: MouseEvent | TouchEvent | PointerEvent, info: { offset: { x: number } }) =>
            isCarouselActive &&
            rotation.set(rotation.get() + info.offset.x * 0.05)
          }
          onDragEnd={(_: MouseEvent | TouchEvent | PointerEvent, info: { velocity: { x: number } }) =>
            isCarouselActive &&
            controls.start({
          ...
          {cards.map((imgUrl, i) => (
            ...
            >
              <motion.img
                ...
                alt={`Screenshot ${i + 1}`}
                layoutId={`img-${imgUrl}`}
                className="pointer-events-none w-full rounded-xl object-contain h-[400px]"
...
export function ThreeDPhotoCarousel() {
  ...
  const cards = useMemo(() => screenshots, [])
  ...
  return (
    <motion.div layout className="relative min-h-screen flex items-center justify-center">
